...
struct CardContent: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        let (_, _, shading, _) = card.allFeaturesAsEnums
    ...
    private func getDrawingPoints(in rect: CGRect) -> [CGPoint] {
        ...
        let numberOfObjects = CGFloat(card.allFeaturesAsRawValues[0])
    ...
    private func draw(the card: SetCard, in rect: CGRect) -> Path {
        var path = Path()
        let points = getDrawingPoints(in: rect)
        let (_, shape, _, _) = card.allFeaturesAsEnums
        switch shape {
        case .diamond:
